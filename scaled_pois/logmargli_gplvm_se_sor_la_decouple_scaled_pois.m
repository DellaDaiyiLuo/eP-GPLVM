function [L,dL] = logmargli_gplvm_se_sor_la_decouple_scaled_pois(uu,yy,BBwfun,ff,covfun,sigma2,nf,BBwTfun,xgrid,cuu,cuuinv,cufx_old,invcc_old,C)
[nt,nneur] = size(ff);
uu = reshape(uu,[],nf);
xx = BBwfun(uu,0);
...
for nn=1:nneur
    ...
    cf0 = cufx*ff0;
    cc = cuuinv*cufx;
    ef0 = C(nn)*exp(cc'*cf0+sigma2*ff0);
    cfyn = cufx*yn;
    cfef0 = cufx*ef0;
    ccf0 = cuuinv*cf0;
    ...
    % exp(f)
    etrm = etrm + C(nn)*sum(exp(cufx'*(cc*ff0)+sigma2*ff0));
