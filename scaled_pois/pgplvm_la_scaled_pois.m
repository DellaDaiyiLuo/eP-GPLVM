function result = pgplvm_la_scaled_pois(yy,nf,setopt,xx)
...
C = ones(1,nneur); % DL: poisson scaler for each cell
ymax = max(vec(yy));
beta = norm(mean(yy,1))^2/nneur; % C~N(0,beta)
...
for iter = 1:niter
    ...
    %% 1. Find optimal ff
     ...
     lmlifun_poiss = @(ff) StateSpaceModelsofSpikeTrains_scaled_pois(ff,yy,cufx,cuu,sigma2,C);
...
%     subplot(412),plot([ff(:,20),exp(ffnew(:,20))]),title('exp(ff)'),legend('true ff','P-GPLVM ff'),drawnow
    ...
    switch ffTYPE
        ...
        case 2 % SE with grad
            switch la_flag
                ...
                case 3
                    % decouple la
                    lmlifun = @(u) logmargli_gplvm_se_sor_la_decouple_scaled_pois(u,yy,Bfun,ffmat,covfun,sigma2,nf,BTfun,xgrid,cuu,cuuinv,cufx_old,invcc_old,C);
            end
    end
    ...
    switch nf
        ...
        case 2
            if mod(iter,2)==0
                plot(xxsamp(1:n1,1),xxsamp(1:n1,2),'Color',[0.1*iter/2,0,0])
                plot(xxsamp(n1+1:end,1),xxsamp(n1+1:end,2),'Color',[0,0.1*iter/2,0.1*iter/2])
    %             scatter(xxsamp(:,1),xxsamp(:,2),3,xx); 
        ...
        case 3
            if mod(iter,2)==0
                scale = 1/niter*iter;
                plot3(xxsamp(1:n1,1),xxsamp(1:n1,2),xxsamp(1:n1,3),'Color',[scale,0,0])
                plot3(xxsamp(n1+1:end,1),xxsamp(n1+1:end,2),xxsamp(n1+1:end,3),'Color',[0,scale,scale])
    %             scatter(xxsamp(:,1),xxsamp(:,2),3,xx); 
                drawnow;
            end
    end
    ...
    if opthyp_flag
        ...
        loghyp0 = [log(hypers),C];
        loghyp = [log([rhoxx;lenxx;rhoff;lenff]);C'];
        lmlifun_hyp = @(loghyp) logmargli_gplvm_se_sor_hyp_scaled_pois(loghyp,loghyp0,xxsamp,xgrid,latentTYPE,tgrid,nt,hypid,sigma2,ffmat,ffTYPE,sigma2,yy, beta);
        opts = optimset('largescale', 'off', 'maxiter', 1e1, 'display', 'off');
        lb = [0;2.3;0;-3;ones(nneur,1)*0.1]; % lower bound
        ub = [5;5;5;5;ones(nneur,1)*ymax]; % upper bound
        loghypnew = fmincon(lmlifun_hyp,vec(loghyp),[],[],[],[],lb,ub,[],opts);
        % loghypnew = fminunc(lmlifun_hyp,vec(loghyp),opts);
        loghyp0new = loghypnew;
        rhoxx = exp(loghyp0new(1));
        lenxx = exp(loghyp0new(2));
        rhoff = exp(loghyp0new(3));
        lenff = exp(loghyp0new(4));
        C = loghyp0new(5:end)';
        display(['iter:' num2str(iter) ', rhoxx:' num2str(rhoxx) ', lenxx:' num2str(lenxx) ', rhoff:' num2str(var(rhoff)) ', lenff:' num2str(var(lenff)) ', sigma2:' num2str(sigma2)])
    end
