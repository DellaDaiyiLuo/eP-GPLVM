...
%     subplot(412),plot([ff(:,20),exp(ffnew(:,20))]),title('exp(ff)'),legend('true ff','P-GPLVM ff'),drawnow
    ...
    switch nf
        ...
        case 2
            if mod(iter,2)==0
                plot(xxsamp(1:n1,1),xxsamp(1:n1,2),'Color',[iter/niter,0,0])
                plot(xxsamp(n1+1:end,1),xxsamp(n1+1:end,2),'Color',[0,iter/niter,iter/niter])
    ...
    if opthyp_flag
        ...
        lb = [0;2.3;0;-3;-5]; % lower bound [0;2.3;0;4;-5];% DL%
        lb = lb(hypid);
    ...
    rhoxx_l = [rhoxx_l,rhoxx];
    lenxx_l = [lenxx_l,lenxx];
    rhoff_l = [rhoff_l,rhoff];
    lenff_l = [lenff_l,lenff];
...
result.lenff = lenff;
result.sigma = sigma2;
result.rhoxx_l = rhoxx_l;
result.lenxx_l = lenxx_l;
result.rhoff_l = rhoff_l;
result.lenff_l = lenff_l;
result.xllh = xllh;
result.fllh = fllh;
